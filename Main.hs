...
quickSort :: [Int] -> [Int]
quickSort [] = []
quickSort (x : xs) = sort' x xs [] []
    where
        sort' s [] ls rs = quickSort ls ++ [s] ++ quickSort rs
        sort' s (x : xs) ls rs = if x < s then sort' s xs (x : ls) rs else sort' s xs ls (x : rs)
...
positions f xs = map' snd  $ filter f' $ zipIndex xs
  where
