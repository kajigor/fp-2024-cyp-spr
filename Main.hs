...
ageOn planet ageInSeconds = case planet of
    "Mercury" -> ageInSeconds / (31557600.0 * 0.2408467)
    "Venus" -> ageInSeconds / (31557600.0 * 0.61519726)
    "Earth" -> ageInSeconds / 31557600.0
    "Mars" -> ageInSeconds / (31557600.0 * 1.8808158)
    "Jupiter" -> ageInSeconds / (31557600.0 * 11.862615)
    "Saturn" -> ageInSeconds / (31557600.0 * 29.447498)
    "Uranus" -> ageInSeconds / (31557600.0 * 84.016846)
    "Neptune" -> ageInSeconds / (31557600.0 * 164.79132)
    "Pluto" -> error "it's not a planet."
    _ -> error "Unrecognised planet"

isLeapYear :: Int -> Bool
isLeapYear year 
    | year < 0 = error "Negative year"
    | mod year 400 == 0 || (mod year 4 == 0 && (mod year 100 /= 0)) = True
    | otherwise = False
